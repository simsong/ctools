...
class Table:
    ...
    def __init__(self, *, name, desc=None, attrib={}, csv_file=None, csv_writer=None, delimiter=None):
        ...
        # name of this table, change spaces to dots
    ...
    def add_variable(self, var):
        ...
        if var.name in self.vardict:
            raise RuntimeError(
                "Variable {} already in table {}".format(var.name, self.name))

        if self.delimiter is not None:
            if var.position is None:
                raise RuntimeError(
                    "Table has delimiter, so variable {} requires a field".format(var))
        ...
        for var in self.vars():
            for r in var.ranges:
                if keyword in r.desc:
                    # we only override with the first element of the range
                    self.overrides[var.name] = r.a
    ...
    def python_class(self, ignore_vars=[]):
        ...
        ret = []
        ret.append("# Automatically generated on {} by {}".format(
            time.asctime(), os.path.abspath(__file__)))
        ...
        ret.append("    def validate_pipe_delimited(self,x):".format(
            self.python_name()))
        ret.append("        fields = x.split('|')")
        ret.append("        if len(fields)!={}: return False".format(
            len(self.vars())))
        ...
        ret.append(
            "    __slots__ = [" + ", ".join(["'{}'".format(var.name) for var in self.vars()]) + "]")
        ...
        for (i, v) in enumerate(self.vars(), 0):
            if v.name in ignore_vars:
                continue
            ret.append("        self.{:15} = fields[{}]  # {}".format(
                v.name, i, v.desc))
        ...
        for var in self.vars():
            if (var.column == None) or (var.width == None) or (var.name in ignore_vars):
                ret.append("        self.{:15} = None   # no column information for {}".format(
                    var.name, var.name))
            else:
        ...
        for var in self.vars():
            if var.name in ignore_vars:
                continue
            type_str = 'str' if (
                var.allow_whitespace and var.python_type == int) else var.python_type.__name__
            ret.append(
                f"            {var.name.lower()}=safe_{var.python_type.__name__}(self.{var.name}),")
        ret.append('        )')
        ret.append('\n')
        ...
        ret.append(
            f"        inst: {self.python_name()} = {self.python_name()}()")
    ...
    def sql_schema(self, extra={}):
        ...
        ret = []
        ret.append("-- SQLite3/MySQL compatiable schema generated {} by {}".format(
            time.asctime(), os.path.abspath(__file__)))
        ...
        types = [v.sql_type() for v in self.vars()] + \
            [sql_type_for_python_value(v) for v in extra.values()]
        for (n, d, t) in zip(names, descs, types):
            sep_comma = ',' if n != names[-1] else ''
            sep_comment = '--' if d else ''
            ret.append("   {} {}{} {} {}".format(
                n, t, sep_comma, sep_comment, d))
        ret.append(");")
        return "\n".join(ret)+"\n"
    ...
    def sql_prepare(self):
        ...
        for (i, v) in enumerate(self.vars(), 1):
            ...
            if s == None:
                raise RuntimeError(
                    "Cannot compile {} type {}".format(v.name, v.vtype))
            lines.append(s)
        return "\\\n".join(lines)

    def write_sql_scanner(self, f):
        data_line_len = max(
            [v.column[1] if (v.column is not None) else 0 for v in self.vars()])

        f.write('#define SQL_CREATE "{}"\n'.format(
            self.sql_schema().replace("\n", " ")))
    ...
    def open_csv(self, fname, delimiter=',', extra=[], mode='w', write_header=True):
        ...
        self.csv_writer = csv.writer(
            self.csv_file, delimiter=delimiter, quoting=csv.QUOTE_MINIMAL)
        if write_header:
        ...
        for v in self.vars():
            if v.start is not None and v.end is not None and v.width is not None:
                if v.width - 1 + v.start != v.end:
                    raise RuntimeError(
                        "Specified width did not line up with specified start and end of var {}".format(v.name))
    ...
    def write_dict(self, d):
        ...
        try:
            self.csv_writer.writerow(
                [v.vformat(self.overrides.get(v.name, d[v.name])) for v in self.vars()])
        except KeyError as e:
            logging.error(
                "*****************   KeyError Error in write_dict for table {} **************".format(self.name))
