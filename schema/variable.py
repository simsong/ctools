...
class Variable:
    ...
    __slots__ = ('name', 'python_type', 'vtype', 'desc', 'position', 'column', 'width', 'ranges', 'default',
                 'format', 'prefix', 'attrib', 'allow_whitespace', 'start', 'end', 'allow_null', 'allow_any')
    ...
    def __init__(self, *, name=None, vtype=None, python_type=None, desc="", position=None, column=None, width=None, default=None,
        ...
        # Starting column in the line if this is a column-specified file 0
        self.column = column
        self.start = start
        self.end = end
    ...
    def add_range(self, newrange):
        ...
        if newrange in self.ranges:
            raise RuntimeError(
                "{}: duplicate range: {}".format(self.name, newrange))
    ...
    def random_value(self):
        ...
        if "f" in self.format:   # special case for floating points that specify a range
            ...
            val = "".join(random.choice("ABCDEFGHIJKLMNOPQRSTUVWXYZ")
            ...
            raise RuntimeError("variable:{} vtype:{} width:{} no allowable values or ranges".format(
                self.name, self.vtype, self.width))
    ...
    def python_validator(self):
        ...
        try:
            expr = " or ".join(
                ["({})".format(r.python_expr(self.python_type, self.width)) for r in ranges])
        except ValueError as e:
            logging.error(
                "Cannot create python range expression for variable "+str(self))
            raise RuntimeError(
                "Cannot create python range expression for variable "+str(self))
