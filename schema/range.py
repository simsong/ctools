...
class Range:
    ...
    RANGE_RE_LIST = [
        ...
        # some descriptions mention alphanumeric vals legal
        re.compile(r"^.*(?P<a>\balphanumeric\b)(?P<desc>.*)"),
        # grabs w/Whitespace, and returns none, white handled elsewhere
        re.compile(r"^.*(?P<a>[Ww]hitespace)(?P<desc>.*)"),
        # if val is null, return None
        re.compile(r"^.*(?P<a>\bnull\b)(?P<desc>.*)"),
        # some descs just say integer, this catches that
        re.compile(r"^.*(?P<a>integers?)(?P<desc>.*)"),
        # catch ranges built like 1 to 3
        re.compile(
            r"^(?P<a>\-?\+?\d+.?\d+) ?to ?(?P<b>\-?\+?\d+.?\d+) ?(?P<desc>.*)"),
        re.compile(r"^(?P<a>\d+) ?to ?(?P<b>\d+)(?P<desc>.*)"),
        re.compile(r"^\s*(?P<a>\d+) ?to ?(?P<b>\d+)(?P<desc>.*)"),
        # catch ranges with desc delimited by =
        re.compile(r"^\s*(?P<a>\d+)\s*-\s*(?P<b>\d+)\s*=\s*(?P<desc>.*)"),
        # catch single vals with desc delimited by
        re.compile(r"^\s*(?P<a>\d+)\s*=\s*(?P<desc>.*)"),
        # catch base letter range vals
        re.compile(
            r"^\s?[a-zA-Z:]*\s?(?P<a>\d+)\s*-\s*(?P<b>\d+)\s*=?\s*(?P<desc>.*)"),
        # special ascii
        re.compile(r"^\s*(?P<a>\d+)\s*–\s*(?P<b>\d+)\s*=?\s*(?P<desc>.*)"),
        re.compile(r"^\s*(?P<a>\d+)\s*=?\s*(?P<desc>.*)"),
        # catch base 36 ranges
        re.compile(r"^\s*(?P<a>[A-Z]+\d*)-(?P<b>[A-Z]+\d*)(?P<desc>.*)"),
        # removed star, was ?P<a>[A-Z]+\d*)*, check this hasn't broken things elsewhere
        # catch base 36 single val with description
    ...
    def extract_range_and_desc(possible_legal_value, python_type=str, hardfail=False, width=None):
        ...
        if hardfail:
            raise ValueError("Cannot recognize range in: " +
                             possible_legal_value)
        return None
    ...
    def python_expr(self, python_type, width):
        # Handle OIDTB or other vals that require specific length
        if self.desc is not None and "Length" in self.desc:
        ...
        logging.error(
            "Cannot make python range expression for type %s", str(python_type))
        raise ValueError(
            "Don't know how to create python range expression for type "+str(python_type))
