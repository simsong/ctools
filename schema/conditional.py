...
class Conditional:
    ...
    __slots__ = ('desc', 'attrib', 'condition', 'consequent',
                 'elif_arr', 'alternative', 'indent_spaces')
    ...
    def __init__(self, *, desc="", attrib={}, condition=default_condition, consequent=[],
        ...
        if len(self.elif_arr) > 0 and len(self.alternative) == 0:
            raise ValueError(
                'elif array cannot contain values while alternative does not')
    ...
    def __repr__(self):
        return ''.join([f'Conditional(condition: {repr(self.condition)}, consequent: ',
                        str([repr(exp)
                            for exp in self.consequent]), 'elif_arr: ',
                        str([repr(elif_cond)
                            for elif_cond in self.elif_arr]), ', alternative: ',
