...
def convert_document_to_pdf(infile, TopMargin=None, BottomMargin=None):
    ...
    if sys.platform == 'darwin':
        ...
        if not os.path.exists(SOFFICE_DARWIN):
            ...
            raise RuntimeError(
                "DOCX conversion on MacOS requires LibreOffice to be installed")
        outdir = os.path.dirname(infile)
        cmd = [SOFFICE_DARWIN, '--headless', '--convert-to',
               'pdf', '--outdir', outdir, infile]
    ...
    raise RuntimeError(
        "unknown how to do PDF conversion on {}".format(sys.platform))
