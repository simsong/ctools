...
def square(x):
    """This is the map function. It's going to run on the executors. Log the hostname, the PID and X as a JSON object"""
    from pyspark import SparkContext
    clogging.setup(level='INFO', syslog='True')
    logging.info( json.dumps({'hostname':socket.gethostname(), 'pid':os.getpid(), 'x':x, 'func':'square', 'applicationId':applicationId()}))
    return x*x
...
def myadder(x,y):
    ...
    from pyspark import SparkContext
    clogging.setup(level='INFO', syslog='True')
    logging.info( json.dumps({'hostname':socket.gethostname(), 'pid':os.getpid(), 'x':x, 'y':y, 'func':'myadder', 'applicationId':applicationId()}))
    return x+y
...
if __name__=="__main__":
    ...
    # Dump the lines in the logfile that have my applicationId and collect all of the json objects
    objs = []
