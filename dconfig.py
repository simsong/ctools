...
CONFIG_PATHNAME = os.path.join(
    SRC_DIRECTORY, CONFIG_FILENAME)    # can be changed
...
def setup_logging(*, config, level):
    logfname = "{}-{}-{:06}.log".format(config[SECTION_RUN][OPTION_NAME],
                                        datetime.datetime.now().isoformat()[0:19], os.getpid())
    ...
    logging.info("START %s %s  log level: %s (%s)", sys.executable,
                 " ".join(sys.argv), loglevel, loglevel)
...
def dpath_expand(path):
    ...
    while True:
        ...
        try:
            path = path.replace(
                m.group(1), config_file[SECTION_PATHS][varname])
...
def dopen(path, mode='r', encoding='utf-8'):
    ...
    logging.info("dopen: path:{} mode:{} encoding:{}".format(
        path, mode, encoding))
    path = dpath_expand(path)
